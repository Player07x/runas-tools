  ...
  return (
    <section aria-label="Inventário do personagem" className="@container rounded-b-[22px] rounded-t-none border border-border bg-card p-3 shadow-sm sm:rounded-b-[27px] sm:p-7">
      ...
      <div className="mt-3 space-y-2">
        <div className="hidden grid-cols-[6.5rem_minmax(0,1fr)_5.5rem_minmax(0,1.2fr)_4.75rem_5rem_2.75rem] gap-2 px-3 text-center text-[0.62rem] uppercase tracking-wide text-muted-foreground @min-[56rem]:grid"><span>Uso</span><span>Nome</span><span>Tipo</span><span>Descrição</span><span>Qtd.</span><span>Peso</span><span>Editar</span></div>
        ...
        {items.map((item) => <article key={item.id} className="virtualized-list-item grid grid-cols-[minmax(0,1fr)_auto] gap-2 rounded-[18px] border border-border bg-background/55 p-3 @min-[56rem]:grid-cols-[6.5rem_minmax(0,1fr)_5.5rem_minmax(0,1.2fr)_4.75rem_5rem_2.75rem] @min-[56rem]:items-center @min-[56rem]:p-2">
          <select value={item.usage} onChange={(event) => changeUsage(item, event.target.value as InventoryUsage)} aria-label={`Uso de ${item.name}`} className="col-start-1 row-start-1 h-10 rounded-xl border border-input bg-background px-2 text-xs font-semibold text-foreground outline-none focus:border-ring @min-[56rem]:col-start-auto @min-[56rem]:row-start-auto">{inventoryUsageOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select>
          <button type="button" onClick={() => openItem(item)} className="col-start-1 row-start-2 min-w-0 truncate text-left text-sm font-bold text-foreground hover:text-primary @min-[56rem]:col-start-auto @min-[56rem]:row-start-auto @min-[56rem]:px-2">{item.name}</button>
          <span className="col-start-2 row-start-1 text-right text-xs font-semibold text-muted-foreground @min-[56rem]:col-start-auto @min-[56rem]:row-start-auto @min-[56rem]:text-center">{inventoryTypeLabel(item.type)}</span>
          <p className="col-span-2 col-start-1 row-start-3 min-w-0 truncate text-xs text-muted-foreground @min-[56rem]:col-span-1 @min-[56rem]:col-start-auto @min-[56rem]:row-start-auto @min-[56rem]:px-2">{plainText(item.description) || "Sem descrição"}</p>
          <label className="col-start-1 row-start-4 flex min-w-0 items-center gap-2 text-xs font-semibold text-muted-foreground @min-[56rem]:col-start-auto @min-[56rem]:row-start-auto @min-[56rem]:justify-center"><span className="@min-[56rem]:sr-only">Quantidade</span><input type="number" inputMode="numeric" min={1} step={1} value={item.quantity} onChange={(event) => changeQuantity(item, Number(event.target.value))} aria-label={`Quantidade de ${item.name}`} className="h-9 w-16 min-w-0 rounded-lg border border-input bg-background px-2 text-center text-xs font-bold tabular-nums text-foreground outline-none focus:border-ring" /></label>
          <span className="col-start-2 row-start-4 text-right text-xs font-semibold tabular-nums text-foreground @min-[56rem]:col-start-auto @min-[56rem]:row-start-auto @min-[56rem]:text-center"><span className="@min-[56rem]:hidden">Peso: </span>{formatWeight(calculateItemRealWeight(item, info.scaleMultiplier))} kg</span>
          <button type="button" onClick={() => openItem(item, "edit")} aria-label={`Editar ${item.name}`} className="col-start-2 row-start-2 inline-flex size-10 items-center justify-center justify-self-end rounded-xl text-muted-foreground hover:bg-muted hover:text-foreground @min-[56rem]:col-start-auto @min-[56rem]:row-start-auto"><Pencil className="size-4" /></button>
        </article>)}
      </div>
