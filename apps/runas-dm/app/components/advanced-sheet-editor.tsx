...
function StatisticsSection({ character, update }: { character: Character; update: UpdateCharacter }) {
  ...
  const statGroups: Array<{ title: string; hint: string; tone: string; fields: Array<{ key: keyof Character["stats"]; label: string }> }> = [
    { title: "Vitalidade", hint: "Vida atual e ajustes do máximo", tone: "vitality", fields: [
      { key: "pv", label: "PV atual" }, { key: "pvBonus", label: "Bônus de PV máximo" },
    ] },
    { title: "Proteção e energia", hint: "Recursos gastos durante o encontro", tone: "resources", fields: [
      { key: "pa", label: "PA atual" }, { key: "paBonus", label: "Bônus de PA máximo" }, { key: "paExtra", label: "PA extra atual" },
      { key: "paExtraBonus", label: "Bônus de PA extra" }, { key: "pe", label: "PE atual" }, { key: "peBonus", label: "Bônus de PE máximo" },
      { key: "peTemporary", label: "PE temporário" },
    ] },
    { title: "Controle narrativo", hint: "Determinação, casualidade e foco", tone: "narrative", fields: [
      { key: "determination", label: "Determinação atual" }, { key: "determinationBonus", label: "Bônus de determinação" },
      { key: "casualty", label: "Casualidade atual" }, { key: "casualtyBonus", label: "Bônus de casualidade" },
      { key: "focusCurrent", label: "Foco atual" }, { key: "focusModifier", label: "Modificador de foco" },
    ] },
    { title: "Movimento e carga", hint: "Capacidade física e deslocamento", tone: "movement", fields: [
      { key: "currentLoad", label: "Carga atual" }, { key: "loadBonus", label: "Bônus de carga máxima" },
      { key: "movementBonus", label: "Bônus de deslocamento" },
    ] },
...
function RecordCard({ title, locked = false, onRemove, children }: { title: string; locked?: boolean; onRemove?: () => void; children: React.ReactNode }) {
  const [open, setOpen] = useState(false)
  return <article className={`advanced-record ${open ? "open" : "collapsed"}`}><header><button className="record-toggle" onClick={() => setOpen((value) => !value)} aria-expanded={open}><ChevronDown size={16} /><strong>{title}</strong><small>{open ? "Ocultar detalhes" : "Ver e editar detalhes"}</small></button>{locked ? <span>Protegida</span> : onRemove && <button className="icon-button subtle danger-icon" onClick={onRemove} title="Remover"><Trash2 size={15} /></button>}</header>{open && <div className="advanced-field-grid">{children}</div>}</article>
}
