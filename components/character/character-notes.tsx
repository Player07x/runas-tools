    ...
    <section aria-label="Anotações do personagem" className="rounded-b-[22px] rounded-t-none border border-border bg-card p-2 shadow-sm sm:rounded-b-[27px] sm:p-7">
      ...
      <div className="space-y-2 pt-3">
        ...
        {visibleNotes.map((note) => <article key={note.id} className="virtualized-list-item grid grid-cols-[minmax(0,1fr)_auto_auto] gap-2 rounded-[18px] border border-border bg-background/55 p-2 md:grid-cols-[minmax(6rem,.75fr)_minmax(7rem,1fr)_minmax(10rem,1.7fr)_6.5rem_2.75rem_2.75rem] md:items-center">
          ...
          <button type="button" onClick={() => { setEditingNote({ ...note }); setIsNewNote(false) }} aria-label={`Visualizar e editar ${note.name}`} className="col-start-2 row-start-1 inline-flex size-10 items-center justify-center rounded-xl border border-input bg-background text-muted-foreground transition hover:text-foreground md:col-start-auto md:row-start-auto"><Eye className="size-4" /></button>
          <button type="button" onClick={() => onRemoveNote(note.id)} aria-label={`Deletar ${note.name}`} className="col-start-3 row-start-1 inline-flex size-10 items-center justify-center rounded-xl text-muted-foreground transition hover:bg-destructive/10 hover:text-destructive md:col-start-auto md:row-start-auto"><Trash2 className="size-4" /></button>
        </article>)}
      </div>
